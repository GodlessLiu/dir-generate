...
function mkArrayFiles(parentPath, Files) {
  for (let index = 0; index < Files.length; index++) {
    ...
    if (file.type === 'dir') {
      ...
      if (!fs.existsSync(path.resolve(parentPath, file.filename + utils.startWithPoint(file.type)))) {
        fs.writeFileSync(path.resolve(parentPath, file.filename + utils.startWithPoint(file.type)), temp);
      } else {
        log.logWarning(`${file.filename}已存在，为您跳过此文件创建`);
      }
    }
